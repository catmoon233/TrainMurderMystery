...
public class HeldItemFeatureRendererMixin {
    ...
    public ItemStack tmm$hideNoteAndRenderPsychosisItems(LivingEntity instance, Operation<ItemStack> original) {
        ...
        if (instance instanceof Player player) {
            var eventRes = AllowItemShowInHand.EVENT.invoker().allowShowInHand(player, ret);
            if (eventRes != null) {
                return eventRes;
            }
        }

        if (TMMClient.moodComponent != null && TMMClient.moodComponent.isLowerThanMid()) { // make sure it's only the
                                                                                           // main hand item that's
                                                                                           // being replaced
